...
const ParkingZoneInfoEdit = ({ zone, setEditMode, setZone }) => {
  ...
  } = useGetData({ url: `/employees/employee-demos` });
  ...
  const [coords, setCoords] = useState(zone?.parkingZoneLocation?.coords ?? []);
    ...
    switch (type) {
      case 'delete':
        setResponsiblePersons(
          responsiblePersons.filter((p) => p.workerId !== person.workerId)
        );
        setEmployeesDataModal([...employeesDataModal, person]);
        return;
      case 'add':
        setEmployeesDataModal(
          employeesDataModal.filter((emp) => emp.workerId !== person.workerId)
        );
  ...
  useEffect(() => {
    if (responsiblePersonsData) {
      setEmployeesDataModal(() => {
        const responsiblePersonsId =
          zone && zone?.responsibleWorkers.map((p) => p.workerId);
