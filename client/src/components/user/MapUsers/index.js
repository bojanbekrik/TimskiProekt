...
const MapUsers = () => {
  const { data: fetchedParkingSpaces, isLoadingParkingSpaces } = useGetData({
    url: '/parking-spaces',
  });
  const { data: fetchedSelectZones, isLoading: isLoadingSelectZones } =
    useGetData({ url: '/parking-zones/names' });
  const {
    zone: zoneInfo,
  ...
  const handleSelectZone = async (e) => {
    ...
    if (_selectedZone === 'NONE') {
      ...
      _freeParkingSpaces = _parkingSpaces.filter((p) => !p.taken).length;
      setZoneInfo(zone);
    } else {
  ...
  useEffect(() => {
    ...
    fetchedParkingSpaces &&
      setFreeParkingSpaces(fetchedParkingSpaces.filter((p) => !p.taken).length);
  }, [fetchedParkingSpaces]);
